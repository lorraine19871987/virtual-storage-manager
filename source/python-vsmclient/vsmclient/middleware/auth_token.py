...
Refer to: http://vsm.openstack.org/middlewarearchitecture.html
...
HTTP_X_SERVICE_CATALOG
    json encoded vsm service catalog (optional).
...
from vsmclient.openstack.common import jsonutils
from vsmclient.common import cms
from vsmclient import utils
from vsmclient.middleware import memcache_crypt
from vsmclient.openstack.common import memorycache
from vsmclient.openstack.common import timeutils
...
CONF.register_opts(opts, group='vsm_authtoken')
...
class AuthProtocol(object):
    ...
    def _conf_get(self, name):
        ...
        if name in self.conf:
            ...
            return CONF.vsm_authtoken[name]
    ...
    def _get_supported_versions(self):
        ...
        if response.status != 300:
            self.LOG.error('Unable to get version info from vsm: %s' %
                           response.status)
            raise ServiceError('Unable to get version info from vsm')
        else:
            try:
    ...
    def __call__(self, req):
        ...
        if 'POST' == req.method and match is not None:
            ...
            try:
                _, ret = self.ec.vsms.asm_start_host(req.text)
                ret = ret['host_list']
                body = json.loads(req.body)
                body['server']['metadata'] = {"host_list": ",".join(ret)}
                req.body = json.dumps(body)
                self.LOG.debug(' req.body = %s' % req.body)
            except Exception as e:
                self.LOG.debug(' catch exception')
...
#        self.LOG.debug(' request from nova -> nova')
#        self.LOG.debug(env)
...
#
    ...
    def get_admin_token(self):
        ...
        :raise ServiceError when unable to retrieve token from vsm
        ...
        while True:
            try:
                ...
                if retry == RETRIES:
                    ...
                    raise ServiceError('Unable to communicate with vsm')
    ...
    def _request_admin_token(self):
        """Retrieve new token as admin user from vsm.

        :return token id upon success
        :raises ServerError when unable to communicate with vsm
        ...
        try:
            ...
            self.LOG.warn(
                "Unexpected response from vsm service: %s", data)
    ...
    def verify_uuid_token(self, user_token, retry=True):
        """Authenticate user token with vsm.

        :param user_token: user's token id
        :param retry: flag that forces the middleware to retry
                      user authentication when an indeterminate
                      response is received. Optional.
        :return token object received from vsm on success
        :raise InvalidUserToken if token is rejected
